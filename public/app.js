// 솜씨 — IA 기반 구현
// 0 Landing(비로그인) · 1 분기 · 2 수요 · 3 공급 · 4 Dashboard · 5 My Info · 6 Admin
...
async function boot() {
  try {
    [ROLES] = await Promise.all([fetch("/api/roles").then((r) => r.json())]);
...
const topFlow = (t) =>
  `<div class="topflow"><span class="ti-back" data-act="back">${ic("i-back")}</span><b>${esc(
    t,
  )}</b></div>`;
...
// 데모 계정: a = 이용자(김다솔), b = 운영자
const ACCOUNTS = {
  a: { name: "김다솔", isAdmin: false },
  b: { name: "운영자", isAdmin: true },
};
// 새 세션 시작 시 이전 사용자 흔적 초기화 (전문가 신분·진행 매칭)
...
function goTab(tab) {
  if (tab === "home") return renderLanding(); // 홈 = 솜씨 소개 (로그인 여부 무관)
...
function renderLogin() {
  ...
  const tryLogin = () => {
    if (!doAuth(app.querySelector("#lId").value)) {
      const box = app.querySelector("#lErr");
      box.innerHTML = `<div class="error-msg">아이디는 a 또는 b 예요. (데모 계정)</div>`;
    }
  };
...
function renderIdea() {
  mount(`
    ...
    <div class="scr">
      ...
      <textarea id="idea" rows="5" placeholder="예) 지역 돌봄 사각지대의 독거 어르신을 매일 자동으로 안부 확인하고, 이상 신호가 있으면 복지사에게 알려주고 싶어요.">${esc(
        S.idea,
      )}</textarea>
...
function renderBrief() {
  ...
  mount(`
    ...
    <div class="scr">
      ...
      <button class="cta-o" data-act="reanalyze" style="margin-top:16px">${ic(
        "i-spark",
      )} 수정한 내용으로 다시 분석</button>
  ...
  app.querySelector('[data-act="reanalyze"]').onclick = async () => {
    ...
    if (!ok) {
      S.brief = edited;
      return renderBrief(); // 실패 시 수정값 유지한 채 복귀
    }
      ...
      const cands = role.recommendations
        .map((rec) => {
          ...
          return `<div class="cand2">
            ...
            <button class="btn-join ${joined ? "done" : ""}" data-join="${esc(rec.maker_id)}" data-role="${esc(
            role.role,
          )}" ${joined ? "disabled" : ""}>${joined ? "✓ 합류" : "같이하기"}</button>
          </div>`;
        })
        .join("");
  ...
  app.querySelectorAll("[data-join]").forEach(
    (b) =>
      (b.onclick = async () => {
        ...
        // MVP: 이용자가 같이하기를 누르면 무조건 수락 → 제안 기록 후 바로 상세 공개
        post("/api/propose", {
    ...
    <div class="scr">
      ...
      <div class="dcard">
        <div class="dhead"><span class="av ${color}" style="width:40px;height:40px;font-size:15px">${esc(initial(rec.name))}</span>
          <div><div class="drole">${esc(rec.name)} · ${esc(roleLabel)}${
    rec.verified ? `<span class="shield">${ic("i-shield")}</span>` : ""
  }</div><div class="dmeta">부합 ${rec.score}%</div></div></div>
...
async function renderTeamDone() {
  ...
  const members = [...S.accepted.values()]
    .map(
      ({ rec, roleLabel, color }) =>
        `<div class="mem"><span class="av ${color}">${esc(initial(rec.name))}</span><small>${esc(
          roleLabel.split(/[ ·/]/)[0],
        )}</small></div>`,
    )
    .join("");
  ...
  mount(`
    ...
    <div class="scr">
      ...
      <div class="pick" style="margin-top:6px">
        ${ROLES.map(
          (r) =>
            `<div class="opt-card ${S.supplyRole === r ? "sel" : ""}" data-role="${esc(r)}"><b>${esc(r)}</b></div>`,
        ).join("")}
      </div>
      <div class="grow"></div>
    </div>
...
async function submitMaker() {
  ...
  // 등록한 전문가를 '내 공급자 신분'으로 기억 → 내 정보에서 받은 제안 확인
...
// ════════ 내 프로젝트·매칭 현황 (이용자 내정보) ════════
// ※ NSM/KPI 측정 지표는 운영자 화면(renderAdmin)에만 노출
...
async function renderMe() {
  // 팀원 되기로 등록했다면, 내 전문가에게 온 '받은 제안' 조회
  let inbox = "";
  if (S.myMakerId) {
  ...
  const supplierCard = S.myMakerId
    ? `<div class="dcard" style="margin-bottom:9px">
        <div class="dhead"><span class="av c" style="width:40px;height:40px;font-size:15px">${esc(
          initial(S.myMakerName),
        )}</span>
  ...
  // 팀원으로 하기(공급) 영역: 등록했으면 내 전문가카드+받은제안, 아니면 등록 유도
  ...
  mount(`
    ...
    <div class="scr">
      ...
      <div class="dcard">
        <div class="dhead"><span class="me" style="width:44px;height:44px;font-size:16px">${esc(
          initial(S.userName),
        )}</span>
    ...
    ${tabbar("me")}`);
  bindCommon();
  app.querySelector('[data-act="editp"]').onclick = () =>
    alert("프로필 수정은 데모에서는 준비 중이에요.");
...
async function renderAdmin() {
  ...
  const rows = (list || [])
    .map(
      (m) => `<div class="lrow">
        <div class="lrow-main"><div class="lrow-t">${esc(m.name)} ${
        m.verified ? `<span class="shield">${ic("i-shield")}</span>` : ""
      }</div><div class="lrow-s">${esc(m.role || "-")} · ${esc(m.headline || "")}</div></div>
        <button class="btn-join ${m.verified ? "done" : ""}" data-verify="${esc(m.id)}" data-on="${
        m.verified ? "1" : "0"
      }">${m.verified ? "인증 해제" : "인증 부여"}</button>
      </div>`,
    )
    .join("");
  mount(`
    ...
    <div class="scr">
      ...
      <div class="dash"><div class="dash-grid">${stat(s.projectCount, "프로젝트")}${stat(
    s.expertCount,
    "전문가",
  )}${stat(s.verifiedCount, "인증 전문가")}${stat(s.teamFormedCount, "결성된 팀")}</div></div>
